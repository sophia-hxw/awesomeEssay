...
## 本文方法
这部分先给出基于像素点做目标检测的框架，然后详细说明是怎样使用多层预测来提高recall，解决重叠目标的歧义问题，最后，给出center-ness分支，来过滤低质量检测框，大幅提高模型性能。

### 全卷积一阶段目标检测器
$F_{i}\in R^{HxWxC}$是CNN网络层$i$的特征向量，$s$是到当前层的总stride。输入图像的groundtruth是$B_{i}=(x^{i}_{0}, y^{i}_{0}, x^{i}_{1}, y^{i}_{1}, c^{i}) \in R^{4} x {1,2,...,C}$，此处的$(x^{i}_{0}, y^{i}_{0})$和$(x^{i}_{1}, y^{i}_{1})$是左上角和右下角的顶点，$c^{i}$是bbox包含对象的类别，$C$是当前模型包含的类别总数，在MS-COCO数据集中为80。

对特征层$F_{i}$上得每个位置$(x,y)$，能用$(\lfloor \frac{s}{2} \rfloor+xs, \lfloor \frac{s}{2} \rfloor+ys)$来定位原始输入图片上的点，基本是$(x,y)$点对应感受野的中心。不同于基于anchor的检测器，将原始输入上的点做为bbox的中心点并用于位置偏移量回归，本文方法直接回归bbox到相应位置。换句话说，本文的检测器类似FCN的处理方法直接将位置视为训练样本，而不是anchor方法中的anchor框。
...
第二个则是多个目标的重叠引起的歧义问题，文中给出了minival数据集中正样本的重叠比例，如果不用FPN且使用$P_{4}$特征层则大概有23.16的重叠样本产生歧义，若使用FPN则可快速下降到7.14，因为大多数重叠目标都被对齐到不同的特征层了。其次，如果是同类物体产生重叠，其实不对结果产生影响，因为他们的类别一样，只有对齐到某个位置的目标才不会被漏检。因此，我们只关注属于bbox不同类别的重叠目标。实验数据得出，有了多层做预测，歧义目标率从17.84降到了3.75。为了证明基于FCN的FCOS不会有歧义问题，我们整理了检测到的bbox是属于歧义位置的数量，仅有2.3，如果考虑不同的类别，会下降到1.5。注意，这个1.5并不是无法检测的目标位置，而是与groundtruth的相交面积较小。因此，这些位置是大概率会漏检的位置，实验也表明，这些位置中FCOS的表现并不比基于anchor的方法差。

#### center-ness该不该有
上文中提到，使用center-ness来过滤远离目标中心点的低质量bbox，该方法可以降AP从33.5增长到37.1，高于RetinaNet的35.9，其中，RetinaNet使用了两个IoU阈值来标记bbox的正负标签。center-ness分支能取代这两个超参数，但是，我们也实验验证了，同时使用center-ness和这两个超参数会获得更好的性能。同时，也应该注意到，center-ness同样可以应用到预测的回归计算上，但是却不用引进center-ness分支，但是这种方式对回归向量的准确率却没有提升，所以建议使用单独的center-ness分支。

#### FCOS VS anchor-based检测器
FCOS相比RetineNet有两个较小的变化，其一在新添加的层中使用GN，但是在最后的预测层却不会用GN，这样的策略会使训练过程更加稳定；另一是使用$P_{5}$来生成$P_{6}$和$P_{7}$来取代RetinaNet中标准的$C_{5}$，同样本文也通过实验发现，这种方式使用$P_{5}$可以小幅度提高准确率。

为了验证FCOS可以成为简单且强大的取代anchor的方法，本文去掉了GN，同时，我们使用RetinaNet中的所有超参。一个遗漏的问题是，不确定FCOS是否能辅助提高这些超参数。

增加一些几乎不耗费算力的算子，本文的anchor-free的检测器可以大幅度提高准确率，有了更好的性能和anchor-free检测器的指标参数，本文鼓励社区思考在目标检测中是否需要anchor框？
...
## 5，RPN的拓展
目前为止，本文验证了FCOS可在一阶段检测算法中表现出色，延展思考，FCOS应该可以替换RPN用在类似Faster RCNN等二阶段方法中，本文也实验验证了这一观点。

对比RPN和FPN，我们用FCOS替换基于anchor的方法，在FPN头部增加GN，可以让训练更加稳定，其他对RPN的设置都与官方代码中对FPN的设置相同。实验表明，即使没有center-ness分支，FCOS方法也比原始方法在$AR^{100}$和$AR^{1k}$上领先较多，加上center-ness分支后，FCOS大幅度提高了准确率，具体可见论文中表格6。
