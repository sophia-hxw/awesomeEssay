...
# 7，通用对象检测的最新技术
Pascal VOC2007、VOC2007 和 MSCOCO 是评估检测算法最常用的三个数据集。 Pascal VOC2012 和 VOC2007 是中等规模的数据集，每个图像有 2 或 3 个对象，VOC 数据集中的对象大小范围不大。 对于 MSCOCO，每张图像有近 10 个对象，并且大多数对象是具有大尺度范围的小对象，这给检测算法带来了非常具有挑战性的任务。 
![voc](assets/voc_dec_results.png)
![coco](assets/mscoco_dec_results.png)

# 8，结束语和未来方向
对象检测已得到积极研究，几乎每隔几个月就会报告一次最新的最新结果。然而，仍然有许多开放的挑战。下面我们讨论几个开放的挑战和未来的方向。

- i) 可扩展的提案生成策略。    
  如Sec3.4中所述，目前大多数检测器都是基于锚的方法，并且存在一些限制检测精度的严重缺陷。当前的anchor priors主要是手动设计的，难以匹配多尺度对象，基于IoU的匹配策略也是启发式的。尽管已经提出了一些将基于锚的方法转化为无锚的方法（例如基于关键点的方法）的方法，但仍然存在一些局限性（计算成本高等），还有很大的改进空间。开发无锚方法成为目标检测中的一个非常热门的话题，因此设计一种高效且有效的建议生成策略可能是一个非常重要的研究方向将来。
- ii) 上下文信息的有效编码。    
  上下文可以影响或阻碍视觉对象检测结果，因为视觉世界中的对象具有很强的关系，而上下文对于更好地理解视觉世界至关重要。然而，很少有人关注如何正确使用上下文信息。如何有效地结合对象检测的上下文可能是一个有前途的未来方向。
- iii) 基于自动机器学习（AutoML）的检测。    
  为某项任务设计一个最优的主干架构可以显着提高结果，但也需要大量的工程工作。因此，直接在数据集上学习主干架构是一个非常有趣和重要的研究方向。受 AutoML 对图像分类开创性工作的启发，已经提出了更多相关工作来通过 AutoML解决检测问题，例如学习 FPN 结构和学习数据增强策略，它显示了对基线的显着改进。然而，AutoML 所需的计算资源对于大多数研究人员来说是负担不起的（超过 100 个 GPU 卡来训练单个模型）。因此，开发低计算框架将对目标检测产生很大影响。此外，未来可以探索检测任务的新结构策略（例如提议生成和区域编码）。
- iv) 目标检测的新兴基准。    
  目前MSCOCO是最常用的检测基准测试平台。然而，MSCOCO只有80个类别，对于现实世界中更复杂的场景来说还是太少了。最近，为了收集更丰富的分类信息，提出了一个新的基准数据集 LVIS。 LVIS 包含 164000 张图像 1000+ 类别，总共有 220 万个高质量实例分割掩码。此外，LVIS 模拟了现实世界的低镜头场景，其中存在大量类别，但有时每个类别的数据很少。 LVIS 将在不久的将来为更具挑战性的检测、分割和低样本学习任务打开一个新的基准。
- v) 低射目标检测。    
  具有有限标记数据的训练检测器被称为低样本检测。基于深度学习的检测器通常具有大量参数，因此需要大量数据，需要大量标记数据才能获得令人满意的性能。但是，使用边界框级别的注释标记图像中的对象非常耗时。低样本学习已被积极研究用于分类任务，但只有少数研究集中在检测任务上。例如，多模态自定步检测学习 (MSPLD)解决了半监督学习设置中的低样本检测问题，其中有大规模未标记的数据集可用。 RepMet 采用深度度量学习（DML）结构，联合学习特征嵌入空间和训练集类别的数据分布。但是，RepMet 仅在具有相似概念（动物）的数据集上进行了测试。 Low-Shot Transfer Detector (LSTD) 解决了基于转移学习的low-shot检测问题，该转移学习通过知识正则化将知识从大型带注释的外部数据集转移到目标集。 LSTD 仍然存在过度拟合的问题。低镜头检测任务还有很大的改进空间。
- vi) 检测任务的主干架构。     
  将在大规模数据集上预训练的分类模型的权重应用于检测问题已成为一种范式。然而，分类和检测任务之间仍然存在冲突，因此这不是最佳解决方案。大多数最先进的检测算法都是基于分类主干的，只有少数算法尝试了不同的选择（例如基于 Hourglass Net 的 CornerNet）。因此，如何开发检测感知骨干架构也是未来的一个重要研究方向。
- vii) 其他研究问题。    
  此外，还有一些其他开放的研究问题，例如大批量学习和增量学习。批量大小是 DCNN 训练的关键因素，但在检测任务中尚未得到很好的研究。对于增量学习，如果在没有初始训练数据的情况下适应新任务，检测算法仍然会遭受灾难性遗忘。这些开放的基础研究问题也值得在未来的工作中得到更多关注。

在本次调查中，我们对用于对象检测任务的深度学习技术的最新进展进行了全面调查。 本次调查的主要内容分为三大类：物体检测器组件、机器学习策略、实际应用和基准评估。 我们回顾了近期文献中的大量代表性文章，并以结构化和系统化的方式介绍了对这一重要主题的贡献。 我们希望这项调查可以让读者全面了解深度学习的目标检测，并有可能激发更多关于目标检测技术及其应用的研究工作。
